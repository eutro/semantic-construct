(ns semantic-construct.parser.destructure
  (:refer-clojure :exclude [destructure]))

;; for some reason it's not available in cljs even though it's defined?
                                     ...
                                     (fn [transforms mk]
                                       (if (keyword? mk)
                                         (let [mkns (namespace mk)
                                               mkn (name mk)]
                                           (cond (= mkn "keys")
                                                 (assoc transforms mk #(keyword (or mkns (namespace %)) (name %)))
                                                 (= mkn "syms")
                                                 (assoc transforms mk #(list 'quote (symbol (or mkns (namespace %)) (name %))))
                                                 (= mkn "strs")
